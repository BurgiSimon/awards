...
- After system: the `DESIGN.md` frontmatter values and `src/styles/tokens.css` are identical; two to four colour tokens; the display face is not on the reflex list; `audit.mjs --scope fonts,contrast,surfaces,slop` is clean (no framework purple or cyan, no glow, no halo: X10–X12).
...
- After motion: every `## Motion score` row has a reduced-motion tier and a recipe or "custom"; `audit.mjs --scope motion,slop` is clean (scrubbed tweens on `ease: 'none'`, no global kill, no reflex bounce, idle loops or hover zoom: X13–X15).
...
- Before jury, the slop pass: run `node "${CLAUDE_PLUGIN_ROOT}/scripts/audit.mjs" <dir> --render --json` (static only, with `rendered: unmeasured` noted, when Playwright is missing). Fix every slop, layout and type finding or record it as earned under `## Exceptions`. Then walk the `judge` rows of `## The slop scan` in `${CLAUDE_PLUGIN_ROOT}/references/anti-patterns.md` against the latest captures and fix what you find in one batch. The jury runs the same scan and caps Creativity at 6 when two families remain, so a family found here is cheaper than one found there.
