...
| Cheap ice | matte blocks, no transmission: bevelled edges → key light inside or below → Fresnel rim → bloom with a high luminance threshold → DOF → fog → fine grain → saturation ≈ 0; refraction on one hero object only | [site:igloo] | [recalled high], render teardown |
