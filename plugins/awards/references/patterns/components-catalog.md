...
2. [Arrival and chrome](#arrival-and-chrome): preloader · nav overlay · cursor · sound toggle · chapter jump rail · scrollspy index · edition switcher and search · theme switcher · mode switch · live readouts
3. [Scroll set-pieces](#scroll-set-pieces): scrubbed hero object · pinned chapter · docked media · horizontal rail · sticky stack cards · scroll-drawn path · compare reveal
...
5. [Evidence blocks](#evidence-blocks): spec and metric blocks · map cards · auto-advancing tabs
6. [The close and beyond](#the-close-and-beyond): footer · sentence-form enquiry · 404 · easter eggs · living-utility pages
...
- **Seen in.** Counter with a hold [site:floema-jewelry] [verified]; circular progress ring plus a sound-consent click [site:leo-parpeix] [verified, live bundle 2026-09-18]; drawn gesture [site:why-zero] [verified]; boot readouts [site:usavionix] [verified]; title card [site:son-daven] [verified feature]; `00/24` frame counter [site:the-line] [verified]; vector animation on `Promise.all` [site:lando-norris] [verified]; two-arc ring [site:mont-fort] [verified]; pure-CSS ASCII [site:igloo] [verified]; intro interaction [site:oryzo] [recalled high]; a level meter fed by video ready, `load` and `fonts.ready`, with a 1.8 s floor and a 4.8 s ceiling [site:christoph-nagel] [verified]; a counter capped at 92 until the model loads [site:primesec] [verified]; a progress rule that stays as the nav baseline [site:grids-obys] [verified]; a boot screen shown once per visitor with click-to-skip [site:mensch] [verified]; an intro film sized to the live element's rect and cut to it [site:areebali] [verified]; a curtain that hands off to a hero opening from a rounded inset [site:goats] [verified]; no preloader, with pre-JS CSS hiding only what will animate in [site:nodeck] [verified].
...
- **Refuse.** A spinner, a `setTimeout` counter [site:runrobrun] [site:spasoje] [site:pensatori-irrazionali], a click wall [site:siena] [site:the-boyd], loading every route's assets.
...
- **Seen in.** Overlay teleported to `<body>` with links numbered 01, 02, 03, closing on Escape and on route change [site:leo-parpeix] [recalled medium]; fullscreen navigation [site:son-daven] [verified]; full-screen Menu beside a persistent division switcher [site:mont-fort] [verified]; stacked large labels on mobile [site:shopify-editions-w26] [recalled medium-low]; a vector-driven hamburger morph [site:lando-norris] [verified]; a `role="dialog"` with Escape, a Tab trap and `inert` behind [site:goats] [verified]; a six-column cell grid that fills in and out on mirrored delays [site:runrobrun] [verified].
...
- **Refuse.** A hamburger on desktop when four links fit; an overlay hidden with opacity that stays focusable; a `div` trigger or a panel driven by unlabelled checkboxes [site:siena] [site:noth] [verified].

### Custom cursor
- **Role.** The affordance system — a pointer that names what a target does (drag, play, view, copy) and gives the page mass.
- **Seen in.** Dot at lerp .75 with a ring at .22; ring 1.35× and dot .7× on hover, ring opacity .35 → .8; contextual pill badges; suppressed on `(hover: none), (pointer: coarse)` [site:leo-parpeix] [recalled medium]; `Cursor` plus a `CursorPlane` for reveals [site:the-line] [verified names]; a custom cursor by tag [site:why-zero] [verified]; none in the Floema source [site:floema-jewelry] [verified absence]; a ring that swaps to up and down arrows over invisible prev / next halves of the screen [site:siena] [verified]; a project-name pill at lerp .3, not mounted on touch [site:alectear] [verified].
...
- **Refuse.** Trail glitter, such as fifteen image copies with the system cursor hidden [site:mensch] [verified]; a cursor with no affordance job; `cursor: none` with a disc that vanishes over links [site:robbietilton] [verified].

### Sound toggle
- **Role.** Opt-in ambience and state-bound SFX, with a control that states what it is doing now.
- **Seen in.** An equaliser-bar toggle, ambient at .375 and SFX at .35 [site:leo-parpeix] [recalled medium]; a `Sound: Off` control drawn in the canvas with rate-limited named SFX [site:igloo] [verified]; a 24 × 24 canvas icon, likely audio-reactive [site:mont-fort] [verified element, inferred behaviour]; Howler with consent handling unknown [site:why-zero] [verified library]; a Howler bank muted by default with the choice remembered [site:nodeck] [verified]; Web Audio oscillator clicks behind a toggle that starts muted [site:areebali] [verified]; one recording per chapter with the state spelled in the header [site:gehry-getty] [verified]; the state as a bracketed glyph beside the label, toggled by one key [site:haoqi] [verified].
...
- **Refuse.** Autoplay or sound on by default [site:pensatori-irrazionali] [site:haoqi] [verified]; hover and click audio with no toggle at all [site:spasoje] [site:siena] [verified]; a toggle with no visible state; audio as a load gate.

### Chapter jump rail
- **Role.** Lets a visitor leave a chaptered sequence for the chapter they came for; the felt progress of a world.
- **Seen in.** Numbered chapters with their own affordance [site:mont-fort] [verified]; a sidebar rail with ordered labels, connecting lines and an active state [site:shopify-editions-w26] [recalled medium-low]; an XP counter that makes progress felt [site:why-zero] [verified]; the current chapter number and title as live header text, shortened on phones [site:gehry-getty] [verified]; numbered chapter buttons with a progress rule and a polite live-region announcer [site:christoph-nagel] [verified]; a current-item ticket that opens into the list of all items [site:siena] [verified].
...
- **Seen in.** A low product row whose underline doubles as an autoplay countdown, each choice repainting ground, panel, button, ink and shadow [site:slosh-seltzer] [verified at family level]; four named themes swapped per section [site:leo-parpeix] [recalled medium]; a theme per route tweened on `<html>` [site:floema-jewelry] [verified]; nav theme sampled from the section under the header [site:lando-norris] [verified]; a header contrast toggle that re-points the primary and shadow tokens [site:wodniack] [verified]; one `data-theme` attribute per section setting ground, ink and border, with a hairline only between same-theme neighbours [site:warmnfuzzy] [verified]; the header re-themed per section by attribute [site:okaydev] [site:siteassist] [verified]; a day / night swap crossfaded by `startViewTransition`, applied before first paint, each theme with one event of its own [site:zainabkabira] [verified].
...
- **Refuse.** A switcher on a product without variants; the six flavour hexes; tweening elements one by one; a rAF loop that polls every section's rect forever to re-theme the nav [site:911rennsport] [verified].

### Persistent mode switch
- **Role.** A control that changes what the whole page is showing — the other half of a portfolio, the grid under the layout, an alternate world — and always says which state is on.
- **Seen in.** A two-segment design | lettering pill docked over every route, replaced at the page end by a large centred copy [site:alectear] [verified]; Grid: On / Off laying the real columns over the page, beside an opt-in alternate mode [site:grids-obys] [verified]; bracketed state letters for theme and sound with single-key shortcuts guarded against inputs [site:haoqi] [verified].
- **Anatomy.** A radio group or a `<button aria-pressed>` whose visible label carries the state; the choice persisted; shortcuts printed in the label when they exist.
- **Motion.** A sliding indicator on the routine band (≈ .4 s); a route swap may slide the page by ≈ 10 % in the switch's direction.
- **Accessibility.** One focusable copy at a time; the state in the accessible name; key shortcuts ignored in inputs and with modifiers.
- **Recipe.** `[recipe:theme-swap-tokens]` when the switch is a theme; `[recipe:page-transitions]` when it is a route.
- **Refuse.** Two focusable copies of one switch; a pointer-built widget with no keyboard path [site:grids-obys] [verified]; the pill labels and dock move as drawn.

### Live readouts in the chrome
- **Role.** Fixed labels that change while the visitor is there — a clock, the pointer in the audience's units, a location — so constant chrome proves the page is live.
- **Seen in.** Pointer position mapped to latitude and longitude, printed to four decimals [site:siteassist] [verified]; a local clock with temperature and a pointer x / y readout on a fixed HUD [site:haoqi] [verified]; a DPR-scaled canvas-2D clock as the close [site:spasoje] [verified]; a local clock whose minute digit rolls beside the enquiry [site:the-boyd] [verified].
- **Anatomy.** Small mono labels in fixed corners; values written as text; one listener or one tick on the shared ticker.
- **Motion.** Damped pointer values (`[pattern:motion-vocabulary#damping-math]`); a digit roll of ≈ .8 s at most.
- **Accessibility.** Decorative readouts `aria-hidden`, or a live region that never announces per frame; static on coarse pointers and under reduced motion.
- **Recipe.** None.
- **Refuse.** Raw `mousemove` writes with no damping [site:siteassist] [verified]; the lat / long box, the clock-and-temperature HUD as drawn.
...
- **Seen in.** Scrubbed chapter transitions held by CSS `position: sticky`, 23 scrubbed triggers and no `pin:` at all [site:son-daven] [verified, live source 2026-09-18]; a sticky scene layer per section with static fallback media beneath [site:shopify-editions-w26] [recalled medium-low]; sticky stages with invisible rails, never `pin: true` [site:the-line] [verified]; a 1480 vh sticky stage feeding one progress value to smootherstep windows [site:primesec] [verified]; a footer clip scrubbed by `currentTime` over a distance derived from its duration [site:goats] [verified]; a pinned film that shrinks into a photographed frame while the wall scales down [site:noth] [verified]; a sticky statement whose pin ends exactly where it settles, with a spacer that sets the next section's peek [site:mensch] [verified]; a section held by its bottom edge so the next slides over a finished screen [site:zainabkabira] [verified]. Two cards do use `pin:` — `pin: true` at `scrub: 1` on desktop only [site:bethebuzz] [verified], and `pinSpacing: false` so the next section covers the hero [site:serotoninn] [verified].
...
### Docked media that grows into its section
- **Role.** A reel present from the first frame without taking the hero: parked in a corner, it grows into the section it belongs to.
- **Seen in.** A black thumbnail fixed bottom right, scrubbed to viewport minus gutters between `top 85%` and `top 2rem` with `ease: 'none'`, then released to scroll with its section; desktop only [site:wearedirect] [verified]. The opposite move, a full reel shrinking to a third of the frame [site:noth] [verified].
...
- **Seen in.** Horizontal rails in a long-form page [site:son-daven] [verified]; an arced collections rail in GL with a dial of titles [site:floema-jewelry] [verified, clone]; a rail held without a pin, the section as tall as its overflow and the track moved `y: +T, x: −T` [site:goats] [verified]; an auto-drifting drag rail that stops when the next chapter reaches centre [site:the-boyd] [verified]; one snapped carousel per career row, neighbours dimmed and blurred, spacing set by a fixed peek [site:robbietilton] [verified].
...
- **Seen in.** `StackCards` as a homepage block [site:mindmarket] [verified component; parameters unknown]; three testimonial cards stacked while the section slides up, then fanned over 460 px of scroll [site:zainabkabira] [verified].
...
- **Seen in.** `ScrollPathSection` [site:mindmarket] [verified component]; `svg.createDrawable` [site:animejs] [verified]; a hand-drawn ellipse stroked on the home link [site:floema-jewelry] [verified, clone]; a dashed route threading every chapter to a summit dot [site:to-top] [verified]; a sprite on a motion path with its dashed trail revealed through a mask stroke [site:zainabkabira] [verified]; a pinned line with a follower that becomes the section's badge [site:bethebuzz] [verified].
...
- **Seen in.** A drag handle travelling an SVG path, snapping on release and clicking the real buttons underneath at threshold — no mask, no second render [site:son-daven] [verified, live source 2026-09-18]; an authored torn edge between a colour layer and its greyscale twin, cycled automatically [site:serotoninn] [verified].
...
- **Seen in.** A metadata quartet with a "team of N at studio" line and an award count; an about page as a tabular CV [site:leo-parpeix] [recalled medium]; underlined credit tables and `/ MICRO / LABELS /` [site:the-line] [verified]; a numbered label plus collection title per item [site:floema-jewelry] [verified]; `YR:` / `CL:` / `TC:` / `DF:` rows with a grid / list toggle [site:spasoje] [verified]; numbered lines with years on a small screen instead of thumbnails [site:areebali] [verified].
...
- **Seen in.** A rAF translate with wraparound, mask-edge fade, pause on hover and clickable links [site:seasats] [clone-described]; `BrandMarquees` and `MarqueeBlock` [site:mindmarket] [verified component]; a social marquee [site:slosh-seltzer] [inferred]; CSS-keyframe rows whose duration is derived from measured content width, `max((Σ widths + gaps) / 70, 12)` s, so every row runs at ≈ 70 px/s, the clone `aria-hidden` with `tabIndex -1` [site:boc] [verified]; 40 px per second with a fractional carry for high refresh rates [site:zainabkabira] [verified]; direction following the scroll direction, slowed on small screens [site:siteassist] [verified]; a hover slow-down to `timeScale .35` [site:okaydev] [verified]; one band per discipline in that discipline's colour [site:pensatori-irrazionali] [verified].
...
- **Refuse.** The hero-metric row (audit X08); count-up animation, as in a stat trio [site:primesec] [verified]; a figure without a unit.

### Interactive map cards
- **Role.** Reach or place shown rather than claimed — offices, sites, a network, a route.
- **Seen in.** An infrastructure map with interactive location cards [site:son-daven] [verified inventory]; a `/network` route and wide map asset on MindMarket's live site, whose rendered route was not checked [site:mindmarket] [verified route/token; rendering unknown]; a city map with game-style controls [site:why-zero] [verified]; a season calendar as a 3D track with DOM-anchored markers [site:lando-norris] [verified]; a site model with named zones, each with a figure and an action, on a clamped orbit [site:likova] [verified]; an explorable model with hotspots as one beat inside a chapter [site:gehry-getty] [verified chunk names]. The 48-country `{ name, x, y }` dataset is clone-derived [recalled low], not live-site evidence.
...
### Auto-advancing tabs with a visible clock
- **Role.** A short feature set that plays itself — services, modules, hero slides — while showing how long each item has left.
- **Seen in.** A services accordion advancing every 8 s with a rule filling under the open row, paused on hover [site:wearedirect] [verified]; tabs at 5 and 7 s with a `power3` swap and the visual offset by .25 s [site:siteassist] [verified]; hero tabs at 6 and 4 s with a bar per tab [site:911rennsport] [verified]; scroll-driven tabs whose labels jump the scroll [site:primesec] [verified].
- **Anatomy.** A `role="tablist"` of real buttons; the progress bar is the timer (`scaleX` 0 → 1, linear over the same duration); a click takes over.
- **Motion.** A swap of ≈ .3 s; the incoming visual lags the text by ≈ .25 s.
- **Accessibility.** Roving tabindex with `aria-selected` and arrow keys; the timer pauses on hover, focus, offscreen and reduced motion — none of the four cards does all four.
- **Recipe.** None.
- **Refuse.** `href="#"` tabs with no selected state [site:siteassist] [verified]; autoplay with no pause control [site:911rennsport] [verified].

## The close and beyond

### Designed footer
- **Role.** The last screen, authored like the first; the conversion in the page's register.
- **Seen in.** A footer tagged by the jury [site:seasats] [verified]; one selected by a footer gallery [site:united-carriers] [verified]; a sticky flare-red footer with a closing logo block, live open/closed timings, back-to-top and a credits link [site:the-line] [verified]; an interactive particle footer [site:oryzo] [recalled high]; a three-line CTA in the last theme [site:leo-parpeix] [recalled medium]; a designed footer in the inventory [site:son-daven] [verified]; a wordmark whose letters part around the hovered one [site:mensch] [verified]; the page's one navigation question asked again at full scale [site:alectear] [verified]; a crumple of the last slide into a bin [site:nodeck] [verified]; a mono credits list for every recording and photograph [site:gehry-getty] [verified]; the e-mail as a copy-to-clipboard tile [site:warmnfuzzy] [site:wearedirect] [verified].
...
- **Role.** A contact form that keeps the site's voice: the visitor completes a sentence rather than filling boxes.
- **Seen in.** Prose fragments in the reading serif, each the `<label>` wrapping its own input or radio group [site:the-boyd] [verified]; pill inputs that grow to their content inside a full-screen sheet [site:alectear] [verified class names].
...
- **Seen in.** Tardigrades that appear after dwelling in the zoom section [site:oryzo] [recalled high]; a hint that spawns 3D fruit with one of two sounds [site:leo-parpeix] [recalled medium]; a disco ball and an award-trophy model in the scene manifest, plus a `?debug` panel [site:lando-norris] [verified]; star confetti on click [site:animejs] [verified]; CSS-3D objects to grab and throw into a perspective catcher, and canvas-2D sprites that rain through the about section [site:wodniack] [verified]; a Konami code that plays a sound only when sound is on [site:pensatori-irrazionali] [verified]; an order button that flees the mouse and admits the joke on phones [site:nodeck] [verified]; an opt-in alternate mode that floods the page and releases flourishes [site:grids-obys] [verified]; a redacted employer as a focusable control with a descriptive name [site:haoqi] [verified].
