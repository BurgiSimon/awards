...
- Newer mirrors [verified on each card]: a complete `.sr-only` block with an `h1`, a bio, an `h2` list of project links and an `llms.txt` link under a 100 % canvas [site:jesperlandberg]; a polite live region announcing an operable device's current screen [site:areebali]; a `<noscript>` rule that unstacks a fixed panel stage back into a document [site:christoph-nagel]; a parallel accessible route fed by the same CMS, linked first in the header [site:gehry-getty] — make that link visible to sighted keyboard users too. Misses: thumbnails that exist only as GL textures over empty boxes [site:haoqi]; a display word as unnamed SVG path data [site:bethebuzz]; a word spelled as one button per glyph with no word-level label [site:spasoje] [inferred].

## Keyboard paths for gates

Why: drag, hold and draw gestures had no keyboard evidence in the first reference set, and Why Zero's five gates plus game-style map controls are the clearest case of a story a keyboard user cannot finish [site:why-zero] [inferred high]. Newer cards give keys to reels and devices but rarely give Tab a stop on what is visible (`[pattern:cursor-and-pointer#keyboard-equivalents]`). The audit fails a pointer-only handler [A11].
...
| Wheel accumulator between slides | prev / next buttons and arrow keys; the ring is a `role="progressbar"`; under reduced motion the wheel still advances, instantly | [site:nodeck] disconnects the wheel under reduced motion [verified] |
| Power-on device | P, Enter or Space; a legend printing only the live keys | [site:areebali] [verified]; no visible focus ring yet |
| Click-to-enter gate | a focusable enter button with an Enter path; nothing behind it reachable while it is up | [site:the-boyd] lets Tab reach the menu behind a pointer-only gate [verified] |
| Reel of GL cards | cards as focusable links in a list, arrow keys stepping one | [site:jesperlandberg] [site:robbietilton] key the reel but leave the cards unfocusable [verified] |
| Scroll-driven or autoplay tabs | `<button role="tab">` with `aria-selected` and arrow keys; autoplay pauses on focus | [site:primesec] `div` tabs, [site:siteassist] `href="#"` tabs [verified] |
...
- Seen since [verified on each card]: a `role="dialog"` menu with `aria-modal`, Escape, a Tab trap, `inert` behind and Lenis stopped [site:goats]; a collapsed nav kept `inert`, Escape closing every layer [site:mensch]; legal iframe dialogs with a focus trap [site:christoph-nagel]; native `<dialog>` everywhere [site:nodeck]; a layer stack where Escape closes the topmost [site:jesperlandberg]. Misses: a menu with no Escape, no `aria-expanded` and no focus move [site:wearedirect]; a panel driven by unlabelled checkboxes [site:siena].
...
- Other live regions: the sound state on toggle, the chapter name under a virtual scroll (a polite announcer naming each panel [site:christoph-nagel] [verified]), a "copied" confirmation after a copy badge, form errors and results (`role="status"` on a newsletter [site:jesperlandberg], `aria-live` on an analyser's result [site:christoph-nagel], both [verified]). One polite region per page; assertive only for errors.
...
- `:focus-visible` styled from the tokens — `outline: 2px solid var(--accent); outline-offset: 4px` in `_shared/base.css` — and never `outline: none` without a replacement [A06] [A07]. The ring belongs to the world the way `::selection` does (The Line's selection is its flare red [site:the-line] [verified]) [S01]: the accent colour, the same offset everywhere, visible on both grounds of a theme swap. Boc's focus outline and skip-link ground are its accent [site:boc] [verified]; Wodniack's `*{outline:none}` with no `:focus` rule is the counter-example, and Accessibility 6.20 was its lowest developer score [site:wodniack] [verified]. Newer rings that belong to their world: the orange mark colour at 2 px [site:mensch]; 3 px black at a 6 px offset [site:nodeck] [verified]. Newer counter-examples: a 2 px white ring on a light-grey page [site:goats]; `outline: none` with no replacement [site:serotoninn] [site:siena] [site:bethebuzz] [site:spasoje] [site:the-boyd] [site:areebali] [verified].
- Every hover state is also a focus-visible state (`[pattern:cursor-and-pointer#keyboard-equivalents]`).
- Heading misses this wave [verified on each card]: links and card titles set as `<h1>`s [site:911rennsport] [site:bethebuzz] [site:to-top]; one `h1` per chapter [site:gehry-getty]; no heading at all in the served HTML [site:haoqi]; no `h1` [site:robbietilton] [site:siena] [site:noth] [site:alectear].

## Contrast

Why: warm near-blacks are the corpus norm and they pass. Son Daven's brass-and-near-black duotone on a light ground is documented at ≈ 5.0:1 by three token extractors [site:son-daven] [verified], though the card does not say which pair was measured — measure your own pairs [C01]. Risks the corpus carries: white at .8 opacity over snow [site:mont-fort] [unknown]; mid-tone HUD labels on a fog gradient [site:igloo] [unknown]; 9 px micro-labels at weight 440 [site:the-line] [verified size] — a label that small needs ≥ 4.5:1 and a real weight. New failures on record: a statement resting at ≈ 1.2:1 before its scroll fill and a 12 %-white heading [site:goats]; 9 px grey chrome on a grey device [site:areebali]; 9.6 px corner labels under a vw-lock [site:jesperlandberg] [verified].
...
- A section switcher taxes every read: 500 px of wheel per slide [site:nodeck]; a wheel lock over panels whose copy must be scrolled inside a box, with Space captured on every target so a focused button cannot be pressed [site:christoph-nagel] [verified]. A snap deck that keeps native scroll in sync so the scrollbar still drags is the lighter form [site:zainabkabira] [verified].
...
- `user-select: none` on the body [site:robbietilton] [verified]; reveals that stay invisible until observed, so an anchor jump lands on an empty screen [site:zainabkabira] [verified].
