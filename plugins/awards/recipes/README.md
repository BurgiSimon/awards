...
| Reel from the corner | `docked-media-grow` | P1 | A fixed card docked bottom right, scaled (transform only, `transform-origin` bottom right) from `300 px` to the viewport minus the gutter between `top 85%` and `top top` of its section, linear `scrub: true`, then swapped from `position: fixed` to the section's flow where the two boxes coincide; unscaled cue link fades out, desktop only (≥ 1024 px), CSS placeholder film so no media ships, full-size in its section under reduced motion | wearedirect, noth |
...
| Loader into hero | `preloader-aperture-handoff` | P1 | Curtain gated on fonts + the hero's first frame (4 s ceiling, no floor, head-script `is-gated` so neither the hero nor the curtain flashes), lifted `yPercent −100` while the hero opens from `clip-path: inset(30% round 12px)` to `inset(0% round 0px)` and its media scales `.6 → 1` (1.4 s `expo.inOut`); root `overflow: hidden` + `inert` until open, `__awards.ready` after the hand-off, `sessionStorage` repeat-visit skip, Skip button; a curtain fade only under reduced motion, a cut in the static tier | goats, areebali |
...
| Finish the half-scroll | `idle-settle-snap` | P2 | Lenis on the shared ticker; a wheel that rests inside the first viewport (outside a `6 %` dead zone at each end) eases to the nearer end after `160 ms` of quiet counted in ticker time (`.7 s` ease-out cubic, not locked, so a wheel takes over), `900 ms` guard; held while a pointer is down (a press stops it in place), disarmed by keyboard, find-in-page and other native scrolls and never armed by touch; off on coarse pointers and under reduced motion | mensch, goats |
| Snapping carousel | `blur-peek-snap-carousel` | P1 | Centred card sharp, neighbours at `.2` + `blur(6px)` (`.1`, no blur, ≤ 920 px), gap computed from a fixed peek (`10 %` / `14 %` of the width), release speed picks the snap (`> 1.2 px/ms` → `320 ms` crisp curve, slow `450–600 ms` soft curve), rubber-band overscroll returned by a critically damped spring stepped exactly with the shared ticker's `dt`; links in a list with roving `tabindex`, arrow / Home / End keys, a live "n of N"; blur kept but instant, snaps instant under reduced motion | robbietilton, zainabkabira |
...
| Video panel wipe | `video-clip-wipe-panels` | P1 | Two double-buffered media layers (canvases standing in for `<video>`): the hidden one holds the next clip, then opens over the visible one from `clip-path: inset()` in `1 s expo.inOut` (reversed going back) with a 1 px glowing seam written by the same tween; copy out `.34 s power2.in` before the gate, in `.72 s power3.out`; stepped wheel, swipe, keys and buttons from the section switcher; `.2 s` cross-fade and parked first frames under reduced motion; README shows the `<video>` swap and preload | christoph-nagel, serotoninn |
...
| Endless work reel | `gl-endless-reel-sheets` | P2 | Virtual offset in items on a modulo-wrapped belt (`mod(i − offset + N/2, N) − N/2`, the first work follows the last going forward), wheel-burst detector (≥ 40 px after a 30 ms gap, 500 ms cooldown, doubled and released at `1 − (1 − .22)^frames`), snap after 200 ms idle, segmented sheets twisted and folded by one `uBend` = velocity · `sin(π·p)` that is exactly 0 at rest, canvas-drawn card art, horizontal belt on landscape and vertical stack on portrait; a button index with `aria-current` and a live "n / 8" counter; flat sheets and one step per input under reduced motion, plain list in the static tier | jesperlandberg, siena |
