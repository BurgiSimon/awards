...
christoph-nagel: double-buffered video clip-wipe chapter change — script.js (switchPanel, prepareNextVideo) — two `<video>`, incoming clip `inset(0 100% 0 0)` → 0 (mirrored going back) over 1.0 s expo.inOut from .12 s; 1 px white seam at opacity .76 with glow `0 0 32px 7px`, moved left 0 → 100% on the same clock and faded at 1.0 s; outgoing copy y −24 / opacity 0, .34 s power2.in, stagger .025; incoming copy y 42 → 0, .72 s power3.out, stagger .065 from .66 s
christoph-nagel: wheel-stepped hero word cycle — script.js (setHeroWord, step) — 3 words, yPercent ±118, .72 s / .78 s power4.inOut, .04 s offset; wheel threshold |deltaY| ≥ 18, 760 ms lock; swipe threshold 55 px
christoph-nagel: VU-meter preloader on real signals — script.js (runPreloader, finishPreloader) — 20 segments, red zone ≥ 85 %; waits for video canplaythrough/loadeddata + window load + fonts.ready; target min(86, 18 + t·0.034) before ready, then 99.6; pace .042 / .105 per frame, dt-scaled; sin(now/88) jitter up to 1.25; minimum 1 800 ms, maximum 4 800 ms; exit glass scale .94 over .48 s power3.in, then clip `inset(50% 0 50% 0)` over .68 s expo.inOut; video un-blurs over 1.05 s
christoph-nagel: glass scroll-badge cursor — script.js (renderCursor) + styles.css (.scroll-cursor) — lerp .2, 3.75 rem, backdrop blur 13px; relabels to "Scroll zurück" on the last panel; the legal iframes post pointer coordinates back to the parent
christoph-nagel: CSS film grain — styles.css (.grain) — 170 px PNG tile, opacity .16, overlay blend, contrast 1.42, 6-step jitter in 0.18 s steps(1); off under reduced motion
christoph-nagel: code-graded footage — styles.css (.background-video) — grayscale(1) contrast(1.04) brightness(.62), scale 1.025; while loading blur(12px), brightness .35, scale 1.08
christoph-nagel: WAAPI fallback for every tween — script.js (animate, setStyles) — cubic-bezier(.2,.75,.2,1) and (.7,0,.2,1) when GSAP fails to load; boot waits for the GSAP script's load, its error, or a 900 ms timeout
christoph-nagel: gsap@3.12.5 (core only, no plugins) — gsap.min.js banner
christoph-nagel: no framework, no bundler (plain script and link tags) — index.html
siena: GL film-strip roll — app.js (sc.roll, fragment Ff) — scrollSpeed .001, lerpFactor .1, snapThreshold .25, snapFactor .1, mouseMultiplier .5, touchMultiplier 5.5, keyScrollCooldown 200 ms, modulo wrap over items/2; mask sampled at x × .7168, corner smoothstep(.1,.4) colour-dodge vec3(1,.86,0) at 20 %, vertical smoothstep(.5,.1) vignette, DPR ≤ 2
siena: velocity blur on DOM frames — app.js updatePositions — blur(lspeed × 10 px), lspeed lerp .1, speed × .95 per frame, parallax offset × value × 10 %, will-change only while moving, off on mobile
siena: entry spin — app.js animateIn — current = target = −60, runs down to the first film (about 7 loops of 8)
siena: ticket gate — app.js ll class — ENTER shown after a fixed 3500 ms, preloader_shown cookie 1 h, fade .3 s
siena: first-visit tutorial — app.js Ya class — shown 2 s after entry, captions step every 4800 ms, tutorial_shown cookie 48 h
siena: menu ticket driven by checkboxes — main.css — :has(input:checked), grid-template-rows over --panels-duration .9s, --customEase cubic-bezier(.19,1,.22,1); a var(----customEase) typo falls back to the default easing
siena: hitbox cursor — main.css + app.js onHitbox — SCROLL ring swaps to up/down arrows over the PREV/NEXT halves, .3/.5/.15 s on easeOutQuint cubic-bezier(.23,1,.32,1)
siena: UI sound cues — app.js Pi.lib — Howler, 4 MP3s (cta in/out, menu in/out), volume .5, no mute control
siena: gsap@3.12.5 (ScrollTrigger, SplitText 3.12.5, DrawSVG, Observer, Flip) — app.js
siena: ogl@unknown — app.js
siena: @unseenco/taxi@unknown — app.js, index.html data-taxi
siena: lenis@1.1.16 — app.js, main.css html.lenis
siena: virtual-scroll@unknown — app.js (vs-touchmove-allowed)
siena: howler@2.2.4 — app.js
siena: webflow (jQuery 3.5.1) — index.html
spasoje: tap-to-swap dot-matrix letter hero — index-30265140bde62e73.js — each glyph is a `<button type="button">`; the swap draws a random glyph from a character table; letter, single-change and multiple-change WAVs at volume .5; the reset knob press is .15 s power3.out, y 1, shadow flattened, LED on, reversed after .1 s; an 8 s idle re-roll timer is armed after the entry (`N(8e3)`, behaviour inferred)
spasoje: zero-duration stepped reveals — index-30265140bde62e73.js — `duration: 0`, `ease: "none"` opacity flips; hero glyphs in Fisher–Yates order at .09 s × i; SplitText lines at stagger .15 after .6 s; rows at .1–.15 s on ScrollTrigger `start: "top 70%"`
spasoje: fake-loading counter gate — 896-29d0967fa208ebe9.js — 000→100 over 4 s after a .5 s delay with a `rough({strength:1, points:20, taper:out, randomize:true})` ease string (no EasePack found in the fetched chunks); then D00→DE0→DEV over .5 s after .8 s (desktop) / 1.2 s (mobile); overflow locked on html and body throughout; no storage flag, so it runs on every full load
spasoje: hide-on-scroll nav — 896-29d0967fa208ebe9.js — ScrollTrigger direction flips a paused `y: -200`, `duration: 0` timeline
spasoje: hint blink — index-30265140bde62e73.js — repeat −1, delay 3.2 s, 1.2 s on / .2 s off; stops once a letter has been clicked
spasoje: canvas-2D live clock close — index-30265140bde62e73.js — `getContext("2d")`, canvas scaled by devicePixelRatio, redrawn every rAF, orange second hand
spasoje: unconsented UI sound — index-30265140bde62e73.js, 896-29d0967fa208ebe9.js — 8 WAVs (letter, singleLetters, multipleLetters, hover, toggle, down, up) played through `new Audio` at volume .3–.5, with no toggle
spasoje: next.js (pages router, `__N_SSG`, next/image) — index.html
spasoje: gsap@3.12.5 + ScrollTrigger + SplitText — 94726e6d-c4cbb5dcf369a3dc.js, 285-c3017cee7e50357f.js, index-30265140bde62e73.js
spasoje: gsap ScrollSmoother (smooth 1.2, non-touch) — 896-29d0967fa208ebe9.js, 285-c3017cee7e50357f.js
spasoje: @gsap/react useGSAP (contextSafe, revertOnUpdate) — index-30265140bde62e73.js
spasoje: canvas 2D, no WebGL library — index-30265140bde62e73.js
spasoje: vercel hosting — DNS (cname.vercel-dns.com)
the-boyd: timer gate: count, then click to enter — DHmUQlsp.js — GSAP counter 0→100 over 4 s with roundProps (not a load signal), then a pointer click on `.preloader`; exit autoAlpha 0 over 1.5 s, ticker rows fly xPercent ±30 / yPercent ±100 with a random delay up to .3 s; skipped on repeat visits by sessionStorage `preloaderPlayed`
the-boyd: character splay — chunks/CzjTch_b.js, chunks/Dq1AkkTS.js — perspective 1000 per word; x ±200 × distance from centre, y 60 × index, rotationY −270, rotationZ ±8 × distance; 1.2 s on default-ease; ScrollTrigger start `center bottom`
the-boyd: named CustomEase set — DHmUQlsp.js — default-ease (.31,.13,.11,1), arrow-ease (.83,0,.17,1), mask-ease (.65,0,.35,1); CSS --transition .4s
the-boyd: hero intro + drawn arrow — chunks/CzjTch_b.js — autoAlpha stagger .05 over .6 s on arrow-ease; drawSVG 0→100 % over 1.6 s; photo parallax yPercent −8.33→8.33
the-boyd: auto-drifting drag rail — chunks/HF9kZAw3.js, chunks/CzjTch_b.js — autoScrollSpeed .5, 16 px gap, wraps via modifiers x, scale 1 + |v|·.001 with v clamped to ±100; first slide scrubbed once from yPercent −120 / rotate 20; auto-scroll off once #about-section reaches centre
the-boyd: mask reveal — chunks/CzjTch_b.js — maskSize → "220% 220%, 100%" on mask-ease
the-boyd: scrub parallax utility — chunks/Dq1AkkTS.js — yPercent −s→s, scrub:true, ease none, overflow hidden
the-boyd: cursor label follower — chunks/BF76eLDg.js — a new gsap.to each rAF frame, .4 s default-ease (tween churn, no quickTo)
the-boyd: sentence-form enquiry — index.html, walk/mobile-s75.png — each `<label class="form__field">` wraps its prose fragment and its input; buyer / designer / artist radios
the-boyd: minute-roll clock — chunks/CzVZRVPT.js — yPercent −100 over .8 s on default-ease
the-boyd: nuxt@3.17.5 — DHmUQlsp.js
the-boyd: gsap@3.13.0 (ScrollTrigger, SplitText, CustomEase, Observer, ScrollTo, Text, DrawSVG, MorphSVG) — DHmUQlsp.js
the-boyd: lenis@1.3.4 — DHmUQlsp.js
the-boyd: stripe (loadStripe) — DHmUQlsp.js
the-boyd: pinia — DHmUQlsp.js
