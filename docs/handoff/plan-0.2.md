...
2. Build tier once: `claude plugin eval . --tag build --scaffold --runs 1 --ablation none --no-publish --trust-plugin -j 2 --max-cost-usd 45 --keep-temp --json evals/results/build-r1.json --allow-tools Write Edit WebFetch "Bash(node *)" "Bash(npm *)" "Bash(npx *)" "Bash(ls *)" "Bash(cat *)" "Bash(mkdir *)"`; then `git status -- plugins/awards/evals` clean. Rerun only failed cases after a fix.
